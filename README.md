...
今はJUCEの`GenericAudioProcessorEditor`（全パラメータが自動でスライダー/トグルとして並ぶ
簡易UI）を使っています。動作確認・音作りにはこれで十分ですが、本家のような専用ノブ・
フローダイアグラム付きのグラフィックUIは別途デザインが必要です（Roadmap参照）。
...
4. **カスタムGUI**: パネルレイアウトを`juce::Component`で自作し、ノブ画像やフローダイアグラムを
   描画する。
